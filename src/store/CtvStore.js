...
class CtvStore {
  ...
  registerCtv = async isCollab => {
    try {
      const data = await RepoManager.ctv.registerCtv({
  ...
  updateInfoCTV = async data => {
    try {
      const res = await RepoManager.ctv.updateInfoCTV(data);
  ...
  requestPayment = async () => {
    try {
      const data = await RepoManager.ctv.requestPayment();
  ...
  getGeneralInfoPaymentCtv = async () => {
    try {
      const data = await RepoManager.ctv.getGeneralInfoPaymentCtv();
  ...
  getPaymentCtvHistory = async (isRefresh = false) => {
    ...
    try {
      if (!this.isEnd) {
        var res = await RepoManager.ctv.getPaymentCtvHistory(this.currentPage);
  ...
  getInfoCTV = async () => {
    try {
      const res = await RepoManager.ctv.getInfoCTV();
  ...
  getReportRose = async (isRefresh = false) => {
    ...
    try {
      if (!this.isEnd) {
        var res = await RepoManager.ctv.getReportRose(this.currentPage);
        var list = res.data.data.data;
        this.isLoading = true;
  ...
  getAllCollaborator = async (isRefresh = false, from, to) => {
    ...
    try {
      if (!this.isEnd) {
        var res = await RepoManager.ctv.getAllCollaborator(this.currentPage, from, to);
        var list = res.data.data.data;
        this.isLoading = true;

        if (isRefresh == true) {
          this.ctvRetroduce = list;
        } else {
          this.ctvRetroduce.push(...list);
        }
...
const ctvStore = new CtvStore();

export const ctvStoreContext = React.createContext(ctvStore);
export const useCtvStore = () => React.useContext(ctvStoreContext);
