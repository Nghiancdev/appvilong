...
class OrderAgencyStore {
  ...
  getAllOrder = async (isRefresh, field_by_value, tab, isBack) => {
    ...
    try {
      if (this.isEnd === false) {
        this.isLoading = true;
        const data = await RepoManager.orderAgency.getOrderHistory(
          this.currentPage,
          '',
          'order_status_code',
...
const orderAgencyStore = new OrderAgencyStore();

export const orderAgencyStoreContext = React.createContext(orderAgencyStore);
export const useOrderAgencyStore = () => React.useContext(orderAgencyStoreContext);
