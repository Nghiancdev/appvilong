...
const RegisterCtvCPN = observer(({child}) => {
  useEffect(() => {}, []);
  ...
  return infoCustomer?.is_collaborator == true &&
    infoCustomer?.is_collaborator != null ? (
    child
  ) : (
    <Scaffold
      appbar={<IAppBar title={'Điều khoản Cộng tác viên'}></IAppBar>}
      body={
        <ScrollView>
          <Column padding={10}>
      ...
      bottomNavigationBar={
        <Container
          ...
          marginBottom={5}
          child={
            <IButton text={'Chấp nhận điều khoản'} onPress={() => {
                  navigation.navigate('UPDATE_CTV'); 
            }}></IButton>
          }></Container>
      }></Scaffold>
  );
