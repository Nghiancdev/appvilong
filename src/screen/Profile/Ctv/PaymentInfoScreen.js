...
const PaymentInfoScreen = observer(({route, navigation}) => {
              ...
              {indexTab == 0 ? (
                <ScrollView>
                  <Column>
                    ...
                    <TouchableOpacity onPress={() => {
                      ...
                      <Row padding={20} mainAxisAlignment={'space-between'}>
                        ...
                        <Text>{`${(dataCtv.date_range ?? '') == '' ? 'Chọn' : moment(dataCtv.date_range ?? new Date).format('DD-MM-yyyy')}`}</Text>
                      </Row>
                    </TouchableOpacity>
                    <Divider></Divider>
                    <TextInput
                  ...
                  <Row mainAxisAlignment={'center'}>
                    <ImageOnePicker
                      imageInput={`${infoCtv?.front_card && !infoCtv.front_card.startsWith('https') ? JSON.parse(infoCtv?.front_card) : infoCtv?.front_card || ""}`}
                      limit={1}
                      title={'Ảnh CMND mặt trước'}
                      onReturn={v => {
                        console.log('front_card', v);
                        setDataCtv({...dataCtv, front_card: v[0]});
                      }}></ImageOnePicker>
                    <ImageOnePicker
                      imageInput={`${infoCtv?.back_card && !infoCtv.back_card.startsWith('https') ? JSON.parse(infoCtv?.back_card) : infoCtv?.back_card || ""}`}
                      limit={1}
                      title={'Ảnh CMND mặt sau'}
                      onReturn={v => {
                        console.log('back_card', v);
