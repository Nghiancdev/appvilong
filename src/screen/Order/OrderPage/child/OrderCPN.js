        ...
        <Column>
          ...
          <Row padding={10} mainAxisAlignment={'flex-start'}>
            ...
            <Expanded
              child={
                <Column>
                  <Text>{`${(order?.line_items_at_time ?? []).length == 0
                    ? 'Lỗi sản phẩm'
                  ...
                  <Column>
                    <Row mainAxisAlignment={'space-between'}>
                      ...
                      <Text>
                        {` x ${(order?.line_items_at_time ?? []).length == 0
                          ? 'Lỗi sản phẩm'
            ...
            child={
              <Row>
                {order.order_status_code ==
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      navigation.navigate('ORDER_SUCCESS', { order: order });
                    }}>
                    <Container
                      width={100}
                      height={35}
                ...
                {order.order_status_code != ORDER_STATUS.COMPLETED ? (
                  ...
                  <Pressable onPress={()=> 
                      // console.log("PRODUCT",JSON.stringify(order.line_items_at_time[0].id))
                    navigation.navigate("PRODUCT",  { productId: order.line_items_at_time[0].id })
                  } >
                    <Container
                      width={100}
                      height={35}
